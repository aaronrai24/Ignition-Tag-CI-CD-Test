...
initialize_project() {
	...
	printf '\n\n Updating and renaming file %s.code-workspace... \n' "${project_name}"
	mv ./*.code-workspace "${project_name}".code-workspace

	# Update the code-workspace file with new settings
	cat << EOF > "${project_name}".code-workspace
{
	...
	"settings": {
		...
		"ignitionFlint.sslVerify": false,
		"ignitionFlint.ignitionGateways": [
			{
				"label": "FLCC",
				"address": "https://${project_name}.localtest.me",
				"projectPaths": [
					"./ignition-data/projects/${project_name}"
				],
...
EOF
	printf "Workspace configuration updated successfully.\n"
